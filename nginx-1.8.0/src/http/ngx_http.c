...
ngx_http_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *(ngx_http_conf_ctx_t **) conf = ctx;
    ...
    /*
     *cmcf是 ngx_http_core_module在http块下的全局配置结构体，它的servers成员是一个动态数组
     *保存着所有ngx_http_core_srv_conf_t的指针，从而关联了所有的server块
     */
    cmcf = ctx->main_conf[ngx_http_core_module.ctx_index];

    /*scsfp指向保存所有 ngx_http_core_srv_conf_t 结构体指针的servers动态数组第1个元素*/
    ...
    for (m = 0; ngx_modules[m]; m++) {
        ...
        /*ngx_modules[m]是一个ngx_module_t模块结构体，它的ctx成员对于HTTP模块来说是ngx_http_module_t 接口*/
        module = ngx_modules[m]->ctx;
    ...
    for (s = 0; s < cmcf->servers.nelts; s++) {
        ...
        /*
         *clcf是server块下的ngx_http_core_loc_conf_t 结构体
         */
        clcf = cscfp[s]->ctx->loc_conf[ngx_http_core_module.ctx_index];

        /*
         *将ngx_http_core_loc_conf_t 组成的双向链表按照location匹配字符串进行排序，这个操作是递归的
         *如果某个location块下还具有其他location，那么它的locations链表也会被排序
         */
        ...
        /*
         *根据已经按照location字符串排序过得双向链表，快速的构建静态的二叉查找书
         */
    ...
    ngx_http_module_t *module, ngx_uint_t ctx_index)
{
    ...
    cscfp = cmcf->servers.elts;
    /*注意，这个ctx实在http{}块下的全局ngx_http_conf_ctx_t结构体*/
    ctx = (ngx_http_conf_ctx_t *) cf->ctx;
    saved = *ctx;
    rv = NGX_CONF_OK;

    /*遍历所有的server块下对应的ngx_http_conf_ctx_t结构体*/
    for (s = 0; s < cmcf->servers.nelts; s++) {
        ...
        if (module->merge_srv_conf) {
             ...
             *块下由create_srv_conf方法创建的结构体，而cscfp[s]->ctx->srv_conf[ctx_index]参数
             *是在server{}块下由create_srv_conf方法创建的结构体
             */
        ...
        if (module->merge_loc_conf) {
            ...
            /*
             *clcf是server块下的ngx_http_core_module模块使用create_loc_conf方法产生的ngx_http_core_loc_conf结构体
             *它的locations成员将以双联表的形式关联到所有当前server{}块下的location块
             */
            clcf = cscfp[s]->ctx->loc_conf[ngx_http_core_module.ctx_index];
    ...
    void **loc_conf, ngx_http_module_t *module, ngx_uint_t ctx_index)
{
    ...
    /*如果location链表为空，也就是说，当前server块下没有location块，立刻返回*/
    if (locations == NULL) {
        return NGX_CONF_OK;
    }
         ...
         q = ngx_queue_next(q))
    {
        ...
        /*
         *如果location后的匹配字符串不依靠Nginx自定义的通配符就可以完全匹配的话，则exact指向当前location对应的
         *ngx_http_core_loc_conf_t结构体，否则使用inclusive指向该结构体，且exact的优先级高于inclusive
         */
        clcf = lq->exact ? lq->exact : lq->inclusive;

        /*clcf->loc_conf 这个指针数组里保存着当前location下所有HTTP模块使用create_loc_conf方法生成结构体的指针*/
        ctx->loc_conf = clcf->loc_conf;

        /*调用merge_loc_conf 方法合并srv、loc级别配置项*/
